...
```{r, eval=TRUE}
penguins_subset <- penguins[, c(1, 2, 5)]

penguins_dream <- penguins_subset[penguins_subset$island == "Dream", ]

  # as.data.frame(penguins$species penguins[penguins$island == "Dream", ] penguins$flipper_length_mm))
```
...
```{r, eval=TRUE}
mean(penguins_dream$flipper_length_mm[penguins_dream$species == "Adelie"], na.rm = TRUE)
...
# Find the year with largest avg body mass, and report year, mass
penguins_largest <- penguins %>% 
  group_by(., species, year) %>% 
  summarize(average_size = mean(body_mass_g, na.rm = TRUE)) %>%
  group_by(species) %>%
  filter(average_size == max(average_size))

penguins_largest

# Calculate mean body mass
penguin_mean_g <- mean(penguins$body_mass_g, na.rm = TRUE)

# Use logic for sizing
penguins_sizing <- penguins_years %>% 
  mutate(., group_size = if_else(body_mass_g > penguin_mean_g, paste0(species,"large"), paste0(species, "small")))


# Filter for Not 2007 using exclusion, relocate year to column index 1
...
```{r, eval=TRUE}
penguins_mean_flip <- penguins %>% 
  group_by(species) %>% 
  summarize(avg_flipper_length_mm = mean(flipper_length_mm, na.rm = TRUE))


ggplot(data = penguins %>% group_by(species) %>% summarize(avg_flipper_length_mm = mean(flipper_length_mm, na.rm = TRUE)), aes(y = avg_flipper_length_mm, x = species, fill = species)) + 
  geom_bar(stat = "identity") + 
  scale_fill_manual(values = c("darkorange","purple","cyan4")) +
  labs(x = "Species", y = "Average Flipper\nLength [mm]") + 
...
```{r, eval = TRUE}
ggplot(data = penguins %>% filter(island == "Dream"), mapping = aes(x = bill_length_mm, y = bill_depth_mm)) + 
