...
penguins <- penguins %>% 
  ...
  relocate(., year) %>% 
  mutate(group_size = case_when(body_mass_g > penguin_mean_g ~ "large",
                                body_mass_g <= penguin_mean_g ~ "small",
                                TRUE ~ NA))

penguins_sizing <- penguins %>% 
...
ggplot(data = penguins %>% group_by(species) %>% summarize(avg_flipper_length_mm = mean(flipper_length_mm, na.rm = TRUE)), aes(x = flipper_length_mm, fill = species)) + 
  geom_bar(alpha = 0.5) + 
  scale_fill_manual(values = c("darkorange","purple","cyan4"), 
                    guide = FALSE) +
  theme_minimal()
