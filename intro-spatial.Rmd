...
author: "Caitlin Mothes - Devin Hunt"
date:  "`r Sys.Date()`"
...
# use grepl for finding string including 'Poudre'
# stringr_detect(col, "thing to find")
...
write_sf(poudre_points_sf, "data/poudre_points.shp")
...
```{r}
writeRaster(elevation_crop, "data/elevation_larimer.tif", overwrite = TRUE)
```
...
```{r}
save(poudre_hwy, poudre_points_sf, counties, roads, file = "data/spatial_objects.RData")
```
...
counties_LDP <- counties %>% 
  filter(NAMELSAD %in% c("Larimer County", "Denver County", "Pueblo County"))
...
```{r}
tm_shape(counties_LDP) + 
  tm_polygons(col = "NAME", palette = "RdYlBu")
```
...
```{r}
# Cheat Way
poudre_elev_point <- get_elev_point(poudre_points_sf)

# Correct Way
poudre_elev_point_extract <- extract(elevation_crop, poudre_points_sf)

# Correct Dataframe Way
poudre_elev_point_extract_df <- as.data.frame(poudre_points_sf) %>% 
  mutate(elevation_data = extract(elevation_crop, poudre_points_sf)) %>% 
  unnest(elevation_data)  #Unnest to remove '.' calls 

kableExtra::kable(poudre_elev_point_extract_df)


ggplot(poudre_elev_point_extract_df, aes(y = Elevation, x = name, fill = name)) + 
  geom_bar(stat = "identity", alpha = 0.7) + 
  geom_text(aes(label = Elevation), vjust = 2) + 
  scale_fill_manual(values = c("darkorange","purple","cyan4")) + 
  labs(x = "Location", 
       y = "Elevation [Meters ASL]", 
       fill = "") + 
...
There are 4 features since the tigris::road function captures all line segments with a matching name, however, there are some segments that lead away from the full highway.
The Poudre Canyon Highway has three disconnected branches from the main road. We see on the map that there are two small segments near Gateway Natural Area, and one disconnected segment to the North.
